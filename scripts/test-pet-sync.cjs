// Public sync entry points must propagate protected snapshot failures without success messages.
const fs=require('node:fs'),vm=require('node:vm'),assert=require('node:assert/strict');
function setup(fail=false){const notices=[],data=new Map([['pendingSyncKeys','["students"]'],['lastSyncTime','before']]);let writes=0;const c={console,document:{readyState:'loading',addEventListener(){}},localStorage:{getItem:k=>data.get(k)||null,setItem:(k,v)=>data.set(k,String(v)),removeItem:k=>data.delete(k)},NotificationSystem:{success:m=>notices.push(m),error:m=>notices.push(m)},CloudSafety:{current:()=> 'A',publish:async()=>{if(fail)throw Error('incomplete');writes++},status:()=> 'pending',report:async e=>notices.push(e.message),read:async()=>{throw Error('read failed')}}};c.window=c;vm.createContext(c);vm.runInContext(fs.readFileSync('js/firebase-sync.js','utf8'),c);c.uploadClassProfilesMerged=async()=>{};c.writeCloudSyncInfo=async()=>{};return{c,data,notices,writes:()=>writes};}
(async()=>{
