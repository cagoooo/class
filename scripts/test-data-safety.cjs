...
(async()=>{
 ...
 await test('同步衝突只送同步提醒，不誤報寵物系統失敗',async()=>{const a=setup();await a.s.publish();const b=setup(a.db);await assert.rejects(b.s.publish(),e=>e.code==='sync-conflict');await b.s.report(Object.assign(Error('另一台裝置已更新此班'),{code:'sync-conflict'}),'A',true);assert.equal(b.petErrors.length,0);assert.equal(b.syncConflicts.length,1);});
