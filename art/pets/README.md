...
- 每種 10 張：蛋 1 張；幼年、成長、成熟各有精神飽滿、開心歡呼、安心休息 3 張，共 120 張，另有 1 張共用神祕蛋。盲盒孵化前只顯示共用神祕蛋，不透露種類。
...
網站使用 **3D 預先渲染圖片**，不是 WebGL 即時旋轉模型。種類於首次達到 10 成長值時以瀏覽器加密亂數等機率抽出；圖鑑僅供欣賞，不能手動挑選或重抽。抽出後固定保存。表情由老師選擇，孵化升級回饋使用開心樣態；不另加經濟或成長規則。
