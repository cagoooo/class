...
| 代號 | 建議 | 優先 | 工時 | 價值 |
|---|---|---|---|---|
| **R-B1** | **全站深色模式對比稽核**：✅已在 v3.12.2 完成。稽核並修復導覽列、統計、Excel匯入、抽籤、提示框、同步確認、登入與歡迎彈窗、標籤及排行榜等 11 項低對比問題 | 🔴 P0 | 1 天 | ⭐⭐⭐⭐⭐ |
...
# 第十二章 R-B1 落地後續：主題與無障礙深化的可開發功能（2026-06-17 新增）

> 本章是在 v3.12.2 全站深色模式對比度稽核與優化落地後，為未來在主題、視覺無障礙及課堂特殊環境（如投影機投射）下的視覺體驗深化所提出的建議。

## F. 主題深化與課堂無障礙適配

| 代號 | 建議 | 優先 | 工時 | 價值 |
|---|---|---|---|---|
| **R-F1** | **無障礙對比度 (A11y) 自動化檢查**：在 GitHub Actions 流水線中加入 Lighthouse CI 或 axe-core 掃描腳本，每當代碼變更時自動檢測對比度（確保符合 WCAG AA 級對比標準 4.5:1），防範未來的「淺底淺字/深底深字」併入 main | 🟠 P1 | 1~2 天 | ⭐⭐⭐⭐ |
| **R-F2** | **課堂投影專用「微光模式」 (Dim Projection Theme)**：針對教室大白天的投影環境（投影機對比度低且有環境光干擾），開發一個純黑底 (`#000000`) 搭配高飽和度高亮字（亮黃/亮綠/高對比白）的微光投影主題，提升後排學生的可讀性 | 🔴 P0 | 1 天 | ⭐⭐⭐⭐⭐ |
| **R-F3** | **動態班級主題色適配 (Dynamic Class Theme)**：目前各班級擁有專屬視覺顏色。應讓系統 primary 色調（如加扣分按鈕、標籤外框、主題元件）隨著當前選擇班級的顏色進行動態 CSS 渲染，在深色與淺色模式下皆能維持品牌色調一體化 | 🟡 P2 | 2 天 | ⭐⭐⭐⭐ |
| **R-F4** | **主題切換過渡動畫 (Smooth Transition)**：在切換深色與淺色模式時，為背景和卡片加入平滑過渡的 CSS 動畫（例如 `transition-colors duration-500`），避免瞬間亮度切換造成的眼睛不適，提升沉浸式體驗 | 🟡 P2 | 0.5 天 | ⭐⭐⭐ |
| **R-F5** | **全站 Toast 系統與錯誤警告對比度加強**：統一稽核在弱網或同步錯誤時彈出的 Toast 浮動提示框在深色底上的顯示，確保所有緊急警告均具備足夠的紅色/黃色高對比度 | 🟠 P1 | 0.5 天 | ⭐⭐⭐ |

---

*📝 第十二章 R-B1 落地後續新增於 2026-06-17 ｜ 當前版本 v3.12.2 ｜ 源自全站深色模式對比度稽核與優化*

---

# 第十三章：孤兒資料維運與後台稽核升級（2026-06-18 新增）

> 本章源自「王薇婷」帳號孤兒狀態已處理並恢復綠色的實際維運案例。現有後台已能看見孤兒數與執行勾選救援，但還缺少「處理紀錄、狀態歷程、誤判防護、巡檢提醒」等長期維運能力。以下建議目標是讓孤兒資料從一次性修復，升級成可追蹤、可回溯、可自動提醒的管理流程。

## G. 維運狀態紀錄與稽核追蹤

| 代號 | 建議 | 優先 | 工時 | 價值 |
|---|---|---|---|---|
| **R-G1** | **孤兒狀態處理紀錄表**：每次某位老師從「有孤兒」變成「無孤兒」時，寫入一筆維運紀錄，包含 teacherUid、顯示名稱、處理前孤兒數、處理後孤兒數、處理方式（老師自助 / 管理員救援 / 手動確認）、處理者與時間。像「王薇婷已處理成綠色」這類事件就能被保存，不會只停留在口頭記憶 | 🔴 P0 | 0.5~1 天 | ⭐⭐⭐⭐⭐ |
| **R-G2** | **後台顯示「最近處理時間」與「處理方式」**：在維運後台表格新增欄位，例如「最後孤兒處理：2026-06-18 15:xx / 老師自助」，讓管理者看到綠燈時也知道是一直正常，還是剛剛才修復完成 | 🟠 P1 | 0.5 天 | ⭐⭐⭐⭐ |
| **R-G3** | **孤兒狀態歷程抽屜**：點某位老師可展開時間線，列出最近 10 筆狀態變化：出現孤兒、檢視孤兒、勾選救援、自助修復、恢復綠色。適合追查「為什麼又出現」或「是不是同一批班級反覆變孤兒」 | 🟠 P1 | 1 天 | ⭐⭐⭐⭐ |
| **R-G4** | **一鍵匯出維運巡檢報告**：匯出 CSV / Markdown，欄位包含老師姓名、email、班級數、孤兒數、最後同步、最後處理時間、狀態。可作為每週巡檢紀錄，也方便貼到交接文件 | 🟡 P2 | 0.5 天 | ⭐⭐⭐ |

## H. 防誤救與防復發機制

| 代號 | 建議 | 優先 | 工時 | 價值 |
|---|---|---|---|---|
| **R-H1** | **孤兒班級狀態分類**：不要只顯示「孤兒」二分法，改為「疑似遺失班級」「空班殘留」「已刪除殘留」「需人工判斷」。判斷依據可用學生數、最後修改時間、是否曾在 classProfiles 出現過 | 🔴 P0 | 1~2 天 | ⭐⭐⭐⭐⭐ |
| **R-H2** | **救援前風險提示**：救援勾選面板中，對 0 學生、長期未修改、名稱像測試班的項目加上醒目提示，避免把老師刻意刪除的殘留班級救回 | 🟠 P1 | 0.5 天 | ⭐⭐⭐⭐ |
| **R-H3** | **救援後自動重新掃描**：管理員按下救援後，後台自動重新呼叫統計，確認該老師已變成綠色。若仍有孤兒，直接提示剩餘項目，不讓管理者誤以為完成 | 🟠 P1 | 0.5 天 | ⭐⭐⭐⭐ |
| **R-H4** | **復發偵測標記**：同一位老師在 7 天內再次出現孤兒資料時，加上「復發」標籤，優先追查同步流程或刪班流程，而不是每次只手動救援 | 🟡 P2 | 1 天 | ⭐⭐⭐⭐ |

## I. 老師端自助與提醒

| 代號 | 建議 | 優先 | 工時 | 價值 |
|---|---|---|---|---|
| **R-I1** | **老師端健檢結果回報管理後台**：老師使用「班級健檢與修復」後，將結果寫入 `_meta/healthChecks`，管理後台可看到「老師已自助修復成功」。王薇婷這類由使用者端處理的案例就能自動留痕 | 🟠 P1 | 1 天 | ⭐⭐⭐⭐⭐ |
| **R-I2** | **綠色狀態明確文案**：老師端與管理端統一顯示「無孤兒資料，雲端班級名冊正常」，避免只有綠燈但不知道代表什麼 | 🟡 P2 | 0.25 天 | ⭐⭐⭐ |
| **R-I3** | **巡檢提醒 Badge**：管理後台頂部新增統計卡：「目前橘色帳號數」「今日已恢復綠色帳號數」「需要人工判斷帳號數」，讓維運工作從逐列掃描變成看儀表板 | 🟠 P1 | 0.5~1 天 | ⭐⭐⭐⭐ |
| **R-I4** | **LINE / Email 維運摘要**：每日固定時間推送摘要給管理者：新增孤兒、已恢復綠色、連續 3 天未處理。適合資料規模到 90 位老師以上時使用 | 🟡 P2 | 1~2 天 | ⭐⭐⭐ |

## J. 建議落地順序

| 順序 | 代號 | 功能 | 理由 |
|---|---|---|---|
| 1 | **R-G1** | 孤兒狀態處理紀錄表 | 先把事件留住，後面所有統計與提醒才有資料來源 |
| 2 | **R-H3** | 救援後自動重新掃描 | 低工時、高確定性，直接減少「以為已處理」的風險 |
| 3 | **R-G2** | 顯示最近處理時間與方式 | 讓綠色狀態具備上下文，適合管理後台立即使用 |
| 4 | **R-I1** | 老師端健檢結果回報後台 | 把老師自助修復納入管理視野 |
| 5 | **R-H1** | 孤兒班級狀態分類 | 從「看數量」升級成「看風險」，降低誤救殘留資料 |
| 6 | **R-I3** | 巡檢提醒 Badge | 當帳號數增加時，讓管理者優先處理真正需要看的項目 |

## K. 目前 P0 / P1 / P2 開發建議總表

> 這張表是給後續開發決策用的「快速分級版」。之後每次更新進度或整理優化方向，都應同步維護這類 P0 / P1 / P2 清單，避免只留下長篇建議卻沒有下一步優先順序。

### 🔴 P0：立即優先，先處理資料安全與誤判風險

| 代號 | 建議 | 為什麼是 P0 |
|---|---|---|
| **R-G1** | 孤兒狀態處理紀錄表 | 王薇婷這類「已從橘色恢復綠色」事件需要留痕，否則之後無法回溯是誰、何時、用什麼方式處理 |
| **R-H1** | 孤兒班級狀態分類 | 單純顯示「孤兒」容易誤救老師刻意刪除的殘留班級，應先分成疑似遺失、空班殘留、已刪除殘留、需人工判斷 |
| **R-A3** | 覆蓋前安全護欄 | 防止空白或新裝置把雲端完整資料覆蓋掉，是所有同步系統最該先擋住的高風險情境 |
| **R-C1** | 版本與健康狀態面板 | 老師回報問題時能快速判斷版本、快取、登入帳號、同步與班級數，減少維運誤判 |

### 🟠 P1：近期優先，高收益且適合下一輪開發

| 代號 | 建議 | 為什麼是 P1 |
|---|---|---|
| **R-H3** | 救援後自動重新掃描 | 按下救援後立刻確認是否真的變綠，低工時但能明顯降低「以為完成」的管理風險 |
| **R-G2** | 後台顯示最近處理時間與處理方式 | 綠色狀態需要上下文，管理者才知道是一直正常還是剛修復完成 |
| **R-I1** | 老師端健檢結果回報管理後台 | 老師自助修復後能自動回寫後台，讓管理者不用靠口頭回報判斷狀態 |
| **R-I3** | 巡檢提醒 Badge | 後台頂部直接顯示橘色帳號數、今日恢復綠色數、需人工判斷數，讓維運更像儀表板 |
| **R-A4** | 老師端雲端資料健檢工具強化 | 把目前管理員才能做的孤兒掃描與修復，逐步變成老師可自助、安全可控的工具 |
| **R-F1** | 無障礙對比度自動化檢查 | 防止深色模式或投影環境再次出現低對比問題，適合放進 CI 長期守門 |

### 🟡 P2：有空再做，提升維運效率與長期體驗

| 代號 | 建議 | 為什麼是 P2 |
|---|---|---|
| **R-G3** | 孤兒狀態歷程抽屜 | 很適合追查復發問題，但需先有 R-G1 的紀錄資料才能發揮價值 |
| **R-G4** | 一鍵匯出維運巡檢報告 | 有助於週報與交接，但不影響當下資料安全 |
| **R-H4** | 復發偵測標記 | 可協助找出反覆出問題的帳號或流程，適合在紀錄機制完成後追加 |
| **R-I4** | LINE / Email 維運摘要 | 當教師帳號規模擴大時很好用，但需先確定後台資料模型與通知規則 |
| **R-A6** | 同步前全班級差異總覽 | 對進階維運有幫助，但實作與 UI 複雜度較高，可排在安全護欄之後 |
| **R-E1** | 多班級總覽儀表板 | 提升跨班管理效率，適合資料穩定後再做體驗加值 |

---

*📝 第十三章新增於 2026-06-18 ｜ 當前版本 v3.12.10 ｜ 源自王薇婷帳號孤兒狀態已處理並恢復綠色的維運案例*
